<template>
    <v-card style="width:450px; height:100%;" outlined>
        ...
        <v-card-title v-if="value._links">
            CustomerVoucher # {{decode(value._links.self.href.split("/")[value._links.self.href.split("/").length - 1])}}
        </v-card-title >
        <v-card-title v-else>
            CustomerVoucher
        </v-card-title >        
        ...
        <v-card-text style="background-color: white;">
            ...
            <Number label="VoucherId" v-model="value.voucherId" :editMode="editMode" :inputUI="''"/>
...
<script>
    ...
    export default {
        name: 'CustomervoucherCustomerVoucher',
        components:{
        },
        props: {
        ...
        methods: {
            ...
            async save(){
                try {
                    ...
                    if(!this.offline) {
                        if(this.isNew) {
                            temp = await axios.post(axios.fixUrl('/customerVouchers'), this.value)
                        } else {
